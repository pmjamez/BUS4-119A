...
def searcher(company_name):
    ...
    print("12")
    pdf_filename = f"{company_name}_ESG_report.pdf"
    print("123")
...
def searcherpotion2(company_name):
...
for company_name in company_names:
    ...
    pdf_filepath1 = searcher(company_name)

    if is_pdf_corrupted(pdf_filepath1):
        print("The PDF is not corrupted.")
    else:
        print("The PDF is corrupted.")
        os.remove(pdf_filepath1)
        print("File has been deleted")
        continue

    pdf_document = fitz.open(pdf_filepath1)
    num_pages = pdf_document.page_count

    if num_pages < 10:
        os.remove(pdf_filepath1)
        print("File too small")
        searcherpotion2(company_name)
